...
server_anthropic() { S="$1" _y -r '.servers[strenv(S)].anthropic // false'; }
...
overview() {
    ...
    for s in $(servers); do
        ...
        [[ "$(server_anthropic "$s")" == "true" ]] && mark+=" ${DIM}[other server]${NC}"
...
#
# Tries /model/info FIRST (a LiteLLM-proxy-specific endpoint, not OpenAI-standard — see
# ../../server/scripts/render-litellm-config.sh) because plain GET /v1/models is fixed by the
# OpenAI spec to id/object/created/owned_by ONLY, even when the proxy config carries real
# metadata — max_model_len/root never show up there no matter what the server does. A raw vLLM box
# has no /model/info at all, so this falls back to /v1/models (which vLLM DOES populate directly)
# whenever the first probe 404s, times out, or comes back empty.
probe_models() {
    local url="$1" base="${1%/v1}" body

    body="$(curl -fsS -m "$PROBE_TIMEOUT" "$base/model/info" 2>/dev/null)" || body=""
    if [[ -n "$body" ]]; then
        local out
        out="$(echo "$body" | jq -r '.data[]? | [.model_name, (.model_info.max_model_len // "")] | @tsv' 2>/dev/null)"
...
# is_gateway_url <url> -> true if <url>'s /model/info answers. Only a full server (litellm) has
# this endpoint; a raw vLLM/OpenAI box never does — this is how the wizard tells "another server
# of its own" apart from "just a model" without asking the user to know the difference.
is_gateway_url() {
    local base="${1%/v1}"
    curl -fsS -m "$PROBE_TIMEOUT" "$base/model/info" 2>/dev/null | jq -e '.data' >/dev/null 2>&1
}
...
action_add_server() {
    ...
    # Default to the server this client is already configured to use — the common case (add ONE
    # server, the one you're already pointed at) then needs no typing at all.
    local default_url=""
    [[ -n "${LITELLM_UPSTREAM:-}" ]] && default_url="${LITELLM_UPSTREAM%/}/v1"
    while true; do
        if [[ -n "$default_url" ]]; then
            read -r -p "Base URL incl. /v1 [$default_url]: " url
            url="${url:-$default_url}"
        else
            read -r -p "Base URL incl. /v1 (e.g. http://10.0.0.25:4000/v1; empty = cancel): " url
            [[ -n "$url" ]] || return 0
        fi
        url="${url%/}"
    ...
    # Every server is talked to directly, at its own address — full stop, that's the whole point
    # of adding it. What's figured out automatically here is HOW the bytes are handled once we're
    # there — no question to answer, nothing to get wrong:
    #   same address as LITELLM_UPSTREAM -> it already speaks Claude's wire format, raw bytes.
    #   otherwise, probe /model/info (only a full server like ../server/ answers this, a raw
    #   vLLM/OpenAI endpoint never does) to tell "another full server" apart from "a plain model
    #   server" (the shim translates for the latter — see ../scripts/claude-shim.js).
    local anthropic=false
    if [[ -n "${LITELLM_UPSTREAM:-}" ]] && [[ "$url" == "${LITELLM_UPSTREAM%/}"* ]]; then
        ei "(this matches LITELLM_UPSTREAM, the address this sandbox was set up with — this list"
        ei " is just for naming its models, Claude already talks to it directly)"
    elif is_gateway_url "$url"; then
        anthropic=true
        ei "(this address is itself a full server — Claude will talk to it directly)"
    else
        ei "(this is a plain model server — Claude will talk to it directly too, translating"
        ei " automatically since it doesn't speak Claude's wire format itself)"
    fi
    S="$name" U="$url" A="$anthropic" _yi '.servers[strenv(S)] = {"url": strenv(U), "anthropic": (strenv(A) == "true"), "models": {}}'
