...
//
// Second job (dormant on the server, kept for anyone embedding this file elsewhere) — a model
//   ROUTER for a catalog file (see ../../ideas/model-catalog-configurator.md): with MODELS_FILE set,
//   it resolves the alias a caller asks for against models.json to a real model id + server URL,
//   rewrites `model`, and forwards. The server compose (../compose.yml) never sets MODELS_FILE —
//   the client owns the catalog and role/alias resolution now (see ../../sandbox-client/), and the server
//   always runs the plain single-VLLM_UPSTREAM path below.
//   Aliases, if ever used: brain/opus/fable/claude-* -> roles.brain, vision/sonnet/haiku ->
//   roles.vision, "<server>/<id>" explicit, "<id>" when exactly one server serves it. Unknown ->
//   404 JSON. Without MODELS_FILE (the shipped server config) it always behaves as: single
//   VLLM_UPSTREAM.
//
